...
function Startup()
	...
	SendNUIMessage({ action = 'setVoiceDistance',
...
CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job or ESX.PlayerData.job2 then
			local job
			local job2
			local blackMoney
			local bank
			...
			if ESX.PlayerData.job2.label == ESX.PlayerData.job2.grade_label then
				job2 = ESX.PlayerData.job2.grade_label
			else
			...
			if ESX.PlayerData.job2.grade_name and ESX.PlayerData.job2.grade_name == 'boss' then
				if (Config.ui.showSociety2Money) then

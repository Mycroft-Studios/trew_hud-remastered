...
function Startup()
    ...
    SendNUIMessage({
        ...
        value = LocalPlayer.state.proximity and LocalPlayer.state.proximity.mode or
            "Normal"
    })
    SendNUIMessage({
        action = 'setMoney',
...
AddEventHandler('esx:onPlayerSpawn', function() Startup() end)
...
CreateThread(function()
    ...
    while true do
        ...
        if not Config.ui.showMinimapOnFoot then
            if not vehicle then DisplayRadar(false) end
        end

        if not Config.ui.showMinimapInVehicle then
            if vehicle then DisplayRadar(false) end
        end

        if Config.ui['carHud'] then
            if vehicle and vehicleIsOn then
                local vehicleClass = GetVehicleClass(vehicle)

                if not Config.ui.showMinimapOnFoot and
                    Config.ui.showMinimapInVehicle then
                    DisplayRadar(true)
                end
                ...
                -- Vehicle Lights
                local vehicleVal, vehicleLights, vehicleHighlights =
                ...
                elseif (vehicleLights == 1 and vehicleHighlights == 1) or
                    (vehicleLights == 0 and vehicleHighlights == 1) then
        ...
        if Config.ui.showLocation then
            local player = ESX.PlayerData.ped
            local position = GetEntityCoords(player)
            local zoneNameFull = zones[GetNameOfZone(position.x, position.y,
                                                     position.z)]
            local streetName = GetStreetNameFromHashKey(
                                   GetStreetNameAtCoord(position.x, position.y,
                                                        position.z))
    ...
    while true do
        ...
        if ESX.PlayerData.job then
            ...
            if ESX.PlayerData.job.label == ESX.PlayerData.job.grade_label then
                ...
                job = ESX.PlayerData.job.label .. ': ' ..
                          ESX.PlayerData.job.grade_label
            end
            ...
            if ESX.PlayerData.job.grade_name and ESX.PlayerData.job.grade_name ==
                'boss' then
        ...
        if Config.ui.showVoice then
            AddEventHandler('pma-voice:setTalkingMode', function()
                SendNUIMessage({
                    action = 'setVoiceDistance',
                    value = LocalPlayer.state.proximity and
...
ESX.RegisterInput("cruise", "[HUD] Cruise Control", "keyboard",
                  Config.vehicle.keys.cruiser, function()
    local player = ESX.PlayerData.ped
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleSpeedSource = GetEntitySpeed(vehicle)
    ...
    if vehicleCruiser == 'on' then
        vehicleCruiser = 'off'
        SetEntityMaxSpeed(vehicle, GetVehicleHandlingFloat(vehicle,
                                                           "CHandlingData",
...
ESX.RegisterInput("seatbelt", "[HUD] Seatbelt", "keyboard",
                  Config.vehicle.keys.seatbelt, function()
    local player = ESX.PlayerData.ped
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
    if not (IsPedInAnyVehicle(player, false) and
        GetIsVehicleEngineRunning(vehicle)) then return end
...
ESX.RegisterInput("leftindicator", "[HUD] Left Indicator", "keyboard",
                  Config.vehicle.keys.signalLeft, function()
    local player = ESX.PlayerData.ped
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
    if has_value(vehiclesCars, vehicleClass) ~= true then return end
...
ESX.RegisterInput("rightindicator", "[HUD] Right Indicator", "keyboard",
                  Config.vehicle.keys.signalRight, function()
    local player = ESX.PlayerData.ped
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
    if has_value(vehiclesCars, vehicleClass) ~= true then return end
...
ESX.RegisterInput("bothindicator", "[HUD] Both Indicators", "keyboard",
                  Config.vehicle.keys.signalBoth, function()
    local player = ESX.PlayerData.ped
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
    if has_value(vehiclesCars, vehicleClass) ~= true then return end
...
function trewDate()
    ...
    local date_format =
        Locales[Config.Locale]['date_format'][Config.date.format]
...
exports('setStatus', function(args)
    local playerStatus = {
        action = 'setStatus',
        status = {{name = args['name'], value = args['value']}}
    }

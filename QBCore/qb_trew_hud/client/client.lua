...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function() Startup() end)
        ...
        if PlayerData and PlayerData.job then
            ...
            if PlayerData.job.isboss then
                ...
                QBCore.Functions.TriggerCallback(
                    'qb-bossmenu:server:GetAccount', function(cb)
                        SendNUIMessage({
                            action = 'setMoney',
                            id = 'society',
                            value = tostring(cb)
            ...
            playerStatus['status'][showPlayerStatus] = {
                ...
                value = math.floor(PlayerData.metadata["stress"] > 1 and
                                       (PlayerData.metadata["stress"] /
                                           PlayerData.metadata["stress"]) * 100 or
                                       1)
            }
        end
...
RegisterInput("cruise", "Cruise Control", "keyboard",
...
RegisterInput("seatbelt", "Seatbelt", "keyboard", Config.vehicle.keys.seatbelt,
              function()
    local player = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
...
RegisterInput("leftindicator", "Left Indicator", "keyboard",
...
RegisterInput("rightindicator", "Right Indicator", "keyboard",
...
RegisterInput("bothindicator", "Both Indicators", "keyboard",
